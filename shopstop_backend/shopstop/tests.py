...
class ListModelTest(TestCase):
    ...
    def test_create_new_object(self):
        ...
        with mock.patch('django.utils.timezone.now', return_value=self.mocked_time):
            instance = List.objects.create(name='list 2', group=Group.objects.get(name='Group 1'))
...
class ListItemModelTest(TestCase):
    ...
    def test_save(self):
        with mock.patch('django.utils.timezone.now', mock.Mock(return_value=self.mocked_time2)):
            instance = ListItem.objects.create(name='test_item', list=List.objects.get(name='list 1'))
...
class GroupViewSetTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(username='test', email='test@shopstop.xyz', password='test1234')
        self.user2 = User.objects.create_user(username="pls_invite_me", email='test@gmail.com', password='test1234')
    ...
    def test_join_group(self):
        request = self.factory.post('/groups/{}/add_current_user_to_group/'.format(self.user_group2.pk))
        force_authenticate(request, self.user)
    ...
    def test_invite_to_group(self):
        request = self.factory.post('/groups/{}/invite_user_to_group/'.format(self.user_group2.pk),
